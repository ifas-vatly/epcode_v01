...
!          call to dpsaupd.  IDO will be set internally to
!          indicate the type of operation to be performed.  Control is
!          then given back to the calling routine which has the
!          responsibility to carry out the requested operation and call
!          dpsaupd with the result.  The operand is given in
...
!          Must be 1,2,3,4,5; See under \Description of dpsaupd for the 
!          five modes available.
!
!          IPARAM(8) = NP
!          When ido = 3 and the user provides shifts through reverse
!          communication (IPARAM(1)=0), dpsaupd returns NP, the number
...
!
!          Note: IPNTR(8:10) is only referenced by dpseupd. See Remark 2.
...
!                     dpseupd if RVEC = .TRUE. See Remarks.
!          -------------------------------------------------------------
!          
...
!          subroutine dpseupd uses this output.
...
!                Informatinal error from LAPACK routine dpsteqr.
...
!     with the ARPACK subroutine dpseupd. 
!
!  2. If the Ritz vectors corresponding to the converged Ritz values
!     are needed, the user must call dpseupd immediately following completion
!     of dpsaupd. This is new starting with version 2.1 of ARPACK.
!
...
!
!\Routines called:
!     dpsaup2  ARPACK routine that implements the Implicitly Restarted
!             Arnoldi Iteration.
!     dpstats  ARPACK routine that initialize timing and other statistics
!             variables.
!     wivout   ARPACK utility routine that prints integers.
!     dpsecond  ARPACK utility routine for timing.
!     dpvout   ARPACK utility routine that prints vectors.
!     dplamch  LAPACK routine that determines machine constants.
!
!\Authors
...
!          the Ritz values of OP computed by dpsaupd transformed to
...
!          dpsaupd.  They are not changed by dpseupd.
...
!          of the above information computed by dpseupd.
!          -------------------------------------------------------------
!          IPNTR(8): pointer to the NCV RITZ values of the original system.
!          IPNTR(9): pointer to the NCV corresponding error bounds.
!          IPNTR(10): pointer to the NCV by NCV matrix of eigenvectors
...
!          = -8: Error return from trid. eigenvalue calculation;
!                Information error from LAPACK routine dpsteqr.
!          = -9: Starting vector is zero.
!          = -10: IPARAM(7) must be 1,2,3,4,5.
...
!
!\Routines called:
!     dpsesrt  ARPACK routine that sorts an array X, and applies the
!             corresponding permutation to a matrix A.
!     dpsortr  dpsortr  ARPACK sorting routine.
!     wivout   ARPACK utility routine that prints integers.
!     dpvout   ARPACK utility routine that prints vectors.
!     dpgeqr2  LAPACK routine that computes the QR factorization of
!             a matrix.
!     dplacpy  LAPACK matrix copy routine.
!     dplamch  LAPACK routine that determines machine constants.
!     dporm2r  LAPACK routine that applies an orthogonal matrix in
!             factored form.
!     dpsteqr  LAPACK routine that computes eigenvalues and eigenvectors
!             of a tridiagonal matrix.
!     dpger    Level 2 BLAS rank one update to a matrix.
!     dpcopy   Level 1 BLAS that copies one vector to another .
!     dpnrm2   Level 1 BLAS that computes the norm of a vector.
!     dpscal   Level 1 BLAS that scales a vector.
!     dpswap   Level 1 BLAS that swaps the contents of two vectors.
!
!\Authors
!     Danny Sorensen               Phuong Vu
!     Richard Lehoucq              CRPC / Rice University
...
!     |       dpsaupd stores the B-norm of the last residual   |
...
!     |       The wanted values are in the first NCONV spots. |
!     | NOTE: workl(1:4*ncv) is set by dpsaupd and is not      |
!     |       modified by dpseupd.                             |
!     %-------------------------------------------------------%
!
!     %-------------------------------------------------------%
!     | The following is used and set by dpseupd.              |
...
!     | Upon exit of dpsaupd WORKD(1:N) has    |
!     | B*RESID(1:N).                         |
!     %---------------------------------------%
!
      rnorm = workl(ih)
...
!        %-------------------------------------------%
!        | If KTRORD .ne. NCONV, something is dpong. |
!        %-------------------------------------------%
!
         if (msglvl .gt. 2) then
             call wivout ( 
...
!        |    where the theta are the Ritz values returned by dpsaupd.  |
!        | NOTES:                                                      |
...
!     +  dpsesrt  ARPACK routine that sorts an array X, and applies the
!                corresponding permutation to a matrix A.
!     +  dpsortr  ARPACK sorting routine.
!!    +  wivout   ARPACK utility routine that prints integers.
!!    +  dpvout   ARPACK utility routine that prints vectors.
!?    +  dpgeqr2  LAPACK routine that computes the QR factorization of
!                a matrix.
...
!
!\Description: 
!  Intermediate level interface called by dpsaupd.
!
!\Usage:
!  call dpsaup2 
...
!\Arguments
!
!  IDO, BMAT, N, WHICH, NEV, TOL, RESID: same as defined in dpsaupd.
!  MODE, ISHIFT, MXITER: see the definition of IPARAM in dpsaupd.
!  
...
!          at H(2,1).  The main diagonal is stored in the dpsecond column
!          of H starting at H(1,2). If dpsaup2 converges store the 
...
!          See Data Distribution Note in dpsaupd.
!
!  INFO    Integer.  (INPUT/OUTPUT)
!          If INFO .EQ. 0, a randomly initial residual vector is used.
!          If INFO .NE. 0, RESID contains the initial residual vector,
...
!
!\Routines called:
!     dpgetv0  ARPACK initial vector generation routine. 
!     dpsaitr  ARPACK Lanczos factorization routine.
!     dpsapps  ARPACK application of implicit shifts routine.
!     dpsconv  ARPACK convergence of Ritz values routine.
!     dpseigt  ARPACK compute Ritz values and error bounds routine.
!     dpsgets  ARPACK reorder Ritz values and error bounds routine.
!     dpsortr  ARPACK sorting routine.
!     wivout   ARPACK utility routine that prints integers.
!     dpsecond  ARPACK utility routine for timing.
!     dpvout   ARPACK utility routine that prints vectors.
!     dplamch  LAPACK routine that determines machine constants.
!     dpcopy   Level 1 BLAS that copies one vector to another.
!     dpdot    Level 1 BLAS that computes the scalar product of two vectors. 
!     dpnrm2   Level 1 BLAS that computes the norm of a vector.
!     dpscal   Level 1 BLAS that scales a vector.
!     dpswap   Level 1 BLAS that swaps two vectors.
!
!\Author
!     Danny Sorensen               Phuong Vu
!     Richard Lehoucq              CRPC / Rice University
...
!
!        %-----------------------------------------------------%
!        | dpsaitr was unable to build an Lanczos factorization |
...
!        | in the exact shift case, dpsgets already handles this.   |
!        %---------------------------------------------------------%
!
         if (ishift .eq. 0) 
     $      call dpcopy (
...
!        | After dpsapps is done, we have a Lanczos                 |
...
!        | the first step of the next call to dpsaitr.  |
!        %---------------------------------------------%
!
         cnorm = .true.
         call dpsecond (t2)
...
!-----------------------------------------------------------------------
!
!     IX is dimensioned, hence, it causes error as n=1 and we 
!     compiled the code without the flag -std=legacy (gfortran)
!
!
      subroutine wivout (
...
!
 ...
 1004 format(1x,' ')
      end subroutine wivout 
!
!-----------------------------------------------------------------------
!
      subroutine dpsecond ( t )
      implicit none 
!
      real       
     $   t
...
!  =======
!
!  SECOND returns the user time for a process in dpseconds.
...
!
!  ITRY    Integer.  (INPUT)
!          ITRY counts the number of times that dpgetv0 is called.  
!          It should be set to 1 on the initial call to dpgetv0.
!
...
!\Routines called:
!     dpsecond  ARPACK utility routine for timing.
!     dpvout   ARPACK utility routine for vector output.
!     dplarnv  LAPACK routine for generating a random vector.
!     dpgemv   Level 2 BLAS routine for matrix vector multiplication.
!     dpcopy   Level 1 BLAS that copies one vector to another.
!     dpdot    Level 1 BLAS that computes the scalar product of two vectors. 
!     dpnrm2   Level 1 BLAS that computes the norm of a vector.
!
...
!
!  where OP and B are as in dpsaupd.  The B-norm of r_{k+p} is also
...
!          semi-inner product for the operator OP.  See dpsaupd.
!          B = 'I' -> standard eigenvalue problem A*x = lambda*x
...
!          and the main diagonal in the dpsecond column.
!
!  LDH     Integer.  (INPUT)
!          Leading dimension of H exactly as declared in the calling 
!          program.
...
!
!\Routines called:
!     dpgetv0  ARPACK routine to generate the initial vector.
!     wivout   ARPACK utility routine that prints integers.
!     dpvout   ARPACK utility routine that prints vectors.
!     dplamch  LAPACK routine that determines machine constants.
!     dplascl  LAPACK routine for careful scaling of a matrix.
!     dpgemv   Level 2 BLAS routine for matrix vector multiplication.
!     dpaxpy   Level 1 BLAS that computes a vector triad.
!     dpscal   Level 1 BLAS that scales a vector.
!     dpcopy   Level 1 BLAS that copies one vector to another .
!     dpdot    Level 1 BLAS that computes the scalar product of two vectors. 
!     dpnrm2   Level 1 BLAS that computes the norm of a vector.
...
!        p_{j} = p_{j}/betaj
!     3) r_{j} = OP*v_{j} where OP is defined as in dpsaupd
...
!     dpvout   ARPACK utility routine that prints vectors.
!     dplamch  LAPACK routine that determines machine constants.
!     dplartg  LAPACK Givens rotation construction routine.
!     dplacpy  LAPACK matrix copy routine.
!     dplaset  LAPACK matrix initialization routine.
!     dpgemv   Level 2 BLAS routine for matrix vector multiplication.
!     dpaxpy   Level 1 BLAS that computes a vector triad.
!     dpcopy   Level 1 BLAS that copies one vector to another.
!     dpscal   Level 1 BLAS that scales a vector.
...
!          main diagonal in dpsecond column.
!
!  LDH     Integer.  (INPUT)
!          Leading dimension of H exactly as declared in the calling 
!          program.
...
!
!\Routines called:
!     dpstqrb  ARPACK routine that computes the eigenvalues and the
...
!
!\Routines called:
!     dpsortr  ARPACK utility sorting routine.
!     wivout   ARPACK utility routine that prints integers.
!     dpsecond  ARPACK utility routine for timing.
!     dpvout   ARPACK utility routine that prints vectors.
!     dpcopy   Level 1 BLAS that copies one vector to another.
...
!        | are applied in subroutine dpsapps.                     |
!        %-------------------------------------------------------%
!     
         call dpsortr (
...
!
!  This is mostly a modification of the LAPACK routine dpsteqr.
!  See Remarks.
!
!\Usage:
!  call dpstqrb
...
!     dplae2   LAPACK routine that computes the eigenvalues of a 2-by-2 
!             symmetric matrix.
!     dplaev2  LAPACK routine that eigendecomposition of a 2-by-2 symmetric 
!             matrix.
!     dplamch  LAPACK routine that determines machine constants.
!     dplanst  LAPACK routine that computes the norm of a matrix.
!     dplapy2  LAPACK routine to compute sqrt(x**2+y**2) carefully.
!     dplartg  LAPACK Givens rotation construction routine.
!     dplascl  LAPACK routine for careful scaling of a matrix.
!     dplaset  LAPACK matrix initialization routine.
!     dplasr   LAPACK routine that applies an orthogonal transformation to 
!             a matrix.
!     dplasrt  LAPACK sorting routine.
!     dpsteqr  LAPACK routine that computes eigenvalues and eigenvectors
...
!
!$$      if (icompz.eq.2)
!$$$     $   call dplaset('full', n, n, zero, one, z, ldz)
!
!     *** New starting with version 2.5 ***
!
      if ( icompz .eq. 2 ) then
...
!
   60    continue
         ...
         if ( m .eq. l ) goto 80
!
!        if remaining matrix is 2-by-2, use dplae2 or dplaev2
...
!
         if ( icompz .gt. 0 ) then
            mm = m - l + 1
!$$$            call dplasr('R', 'V', 'B', n, mm, work(l), work(n-1+l),
!$$$     $                  z(1, l), ldz)
!
!             *** New starting with version 2.5 ***
!
...
!
         if ( icompz .gt. 0 ) then
            mm = l - m + 1
!$$$            call dplasr('R', 'V', 'F', n, mm, work(m), work(n-1+m),
!$$$     $                  z(1, m), ldz)
!
!           *** New starting with version 2.5 ***
!
...
!  =======
!
!  WXERBLA  is an error handler for the LAPACK routines.
...
!          The name of the routine which called WXERBLA.
!
!  INFO    (input) INTEGER
...
!
 ...
 9999 format( / / ' warning. the value emin may be incorrect:-',
     ...
     $      ' dplamc2,', / ' otherwise supply emin explicitly.', / )
      end subroutine dplamc2
!
!***********************************************************************
!
      function dplamc3 ( 
...
!
      ...
      if ( z.eq.zero ) then
         ...
         dplapy2 = w*sqrt( one + ( z / w )**2 )
      endif
      return
      end function dplapy2  
!
!
!***********************************************************************
!
...
!
      if ( first ) then
         ...
         safmin = dplamch ('S')
         eps = dplamch ('E')
         safmn2 = dplamch ('B')**int( 
     $            log(safmin/eps) / log( dplamch ('B') ) / two )
         safmx2 = one / safmn2
      endif
      if ( g.eq.zero ) then
         cs = one
...
!  if DPSYTRD or DSPTRD or DSBTRD has been used to reduce this matrix to
!  tridiagonal form.
!
!  Arguments
!  =========
...
!  =======
!
!  dplaruv returns a vector of n random real numbers from a uniform (0,1)
!  distribution (n <= 128).
!
...
!     name, so that
!        DNRM2 := sqrt( x'*x )
!  -- This version dpitten on 25-October-1982.
...
!          where NB is the blocksize for DPSYTRD returned by WILAENV.
!
...
!
      safmin = dplamch ( 'safe minimum' )
      eps = dplamch ( 'precision' )
      smlnum = safmin / eps
      bignum = one / smlnum
      rmin = sqrt( smlnum )
      rmax = sqrt( bignum )
...
!
     ...
     $        n, n, a, lda, info )
!
!     Call DPSYTRD to reduce symmetric matrix to tridiagonal form.
!
      inde = 1
      indtau = inde + n
      indwrk = indtau + n
...
!          dpitten by the corresponding elements of the tridiagonal
...
!
      ...
      return
      end function wieeeck 
!
!***********************************************************************
!
      function wilaenv ( 
     $         ispec, sname, opts, n1, n2, n3, n4 )
!
      implicit none
      include 'epcode_inc_dp.h'
!
...
!               where k is given by WILAENV (2,...) and m by WILAENV (5,...)
...
!
!  SNAME   (input) CHARACTER*(*)
!          The name of the calling subroutine, in either upper case or
!          lower case.
!
!  OPTS    (input) CHARACTER*(*)
!          The character options to the subroutine SNAME, concatenated
...
!  N3      (input) INTEGER
!  N4      (input) INTEGER
!          Problem dimensions for the subroutine SNAME; these may not all
!          be required.
!
! (WILAENV) (output) INTEGER
!          >= 0: the value of the parameter specified by ISPEC
!          < 0:  if WILAENV = -k, the k-th argument had an illegal value.
!
!  Further Details
!  ===============
!
!  The following conventions have been used when calling WILAENV from the
!  LAPACK routines:
!  1)  OPTS is a concatenation of all of the character options to
!      subroutine SNAME, in the same order that they appear in the
!      argument list for SNAME, even if they are not used in determining
!      the value of the parameter specified by ISPEC.
!  2)  The problem dimensions N1, N2, N3, N4 are specified in the order
!      that they appear in the argument list for SNAME.  N1 is used
!      first, N2 second, and so on, and unused problem dimensions are
!      passed a value of -1.
!  3)  The parameter value returned by WILAENV is checked for validity in
!      the calling subroutine.  For example, WILAENV is used to retrieve
!      the optimal blocksize for STRTRI as follows:
!
!      NB = WILAENV ( 1, 'STRTRI', UPLO // DIAG, N, -1, -1, -1 )
...
!
      goto ( 
     $   100, 100, 100, 400, 500, 600, 700, 800, 900, 1000, 1100 ) 
     $   ispec
!
...
!
  100 continue
!
!     Convert SNAME to upper case if the first character is lower case.
!
      wilaenv = 1
!
      subnam = sname(2:)
...
!     indicate our setting for the working precision, and avoid 
!     conflicts to other LAPACK dependences.
!
      ic = ichar( subnam( 1:1 ) )
      iz = ichar( 'Z' )
...
!     d: double precision real kind (conventional def.)   
!     q: quad.  precision real kind (new def.)   
! 
      lcnam = c1.eq.'C' .or. c1.eq.'Z'
!
      if ( .not.( lcnam .or. lsnam ) ) return
      c2 = subnam( 2:3 )
      c3 = subnam( 4:6 )
      c4 = c3( 2:3 )
!
...
!
     ...
     $             c3.eq.'QLF'     ) then
         ...
         else if ( lsnam .and. c3.eq.'TRD' ) then
            nb = 32
         else if ( lsnam .and. c3.eq.'GST' ) then
            nb = 64
         endif
      else if ( lcnam .and. c2.eq.'HE' ) then
         if ( c3.eq.'TRF' ) then
            nb = 64
      ...
      else if ( lsnam .and. c2.eq.'OR' ) then
         if ( c3( 1:1 ).eq.'G' ) then
            if ( c4.eq.'QR' .or. 
     $           c4.eq.'RQ' .or. 
     $           c4.eq.'LQ' .or.
     ...
     $           c4.eq.'BR'     ) then
      ...
      else if ( lcnam .and. c2.eq.'UN' ) then
         if ( c3( 1:1 ).eq.'G' ) then
            if ( c4.eq.'QR' .or. 
     $           c4.eq.'RQ' .or. 
     $           c4.eq.'LQ' .or.
     ...
     $           c4.eq.'BR'     ) then
      ...
      else if ( lsnam .and. c2.eq.'ST' ) then
         if ( c3.eq.'EBZ' ) then
            nb = 1
         endif
      endif
...
!
      wilaenv = int( real( min(n1, n2) )*1.6e0, kind=wip )
      return
!
  700 continue
!
...
!
!  This is an auxiliary routine called by DPSYTRD.
!
!  Arguments
!  =========
!
...
!          TAU(i) must contain the scalar factor of the elementary
!          reflector H(i), as returned by DPSYTRD.
!
!  WORK    (workspace/output) REAL(wrp)        array, dimension (LWORK)
!          On exit, if INFO = 0, WORK(1) returns the optimal LWORK.
!
...
!
      if ( upper ) then
!
!        Q was determined by a call to DPSYTRD with UPLO = 'U'
!
...
!
!        Q was determined by a call to DPSYTRD with UPLO = 'L'.
!
!        Shift the vectors which define the elementary reflectors one
